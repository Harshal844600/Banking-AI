...
import { computeHealthScore, monthlyCashflow, scoreLabel, spendByCategory } from "@/lib/health-score";
    ...
    <div className="px-6 md:px-8 py-8 max-w-7xl mx-auto animate-reveal">
      <div className="flex items-end justify-between mb-8">
        ...
        <div className="hidden md:flex items-baseline gap-1">
          <span className="text-3xl font-mono font-medium tracking-tighter">{fmtCurrency(health.income - health.expenses)}</span>
      ...
      <div className="grid grid-cols-12 gap-6">
        {/* Health score */}
        <div className="col-span-12 lg:col-span-4 bg-background border border-border p-8 flex flex-col items-center justify-center relative">
          <div className="absolute top-4 left-4 label-eyebrow text-muted-foreground italic">Health Metric v1</div>
          <ScoreRing score={health.total} />
          <p className="mt-6 text-xs text-center text-muted-foreground leading-relaxed max-w-[24ch]">
            Savings {health.savings}/40 · Stability {health.stability}/20 · Balance {health.balance}/20 · Cashflow {health.positivity}/20
          </p>
        </div>

        {/* Spending breakdown */}
        <div className="col-span-12 lg:col-span-8 bg-background border border-border p-8">
          <div className="flex justify-between items-start mb-6">
            <h3 className="label-eyebrow">Categorical Burn</h3>
            <Link to="/app/transactions" className="text-xs font-medium text-accent hover:underline">View ledger →</Link>
          </div>
          {byCat.length === 0 ? (
              ...
              {byCat.map(row => (
                <div key={row.category} className="space-y-2">
                  ...
                  <div className="h-1.5 w-full bg-secondary">
                    <div
                      className={row.category === "investments" ? "h-full bg-accent" : "h-full bg-foreground"}
        ...
        {/* Cash flow chart */}
        <div className="col-span-12 lg:col-span-7 bg-background border border-border p-8">
          <div className="flex justify-between items-start mb-6">
          ...
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={cashflow} barGap={2}>
                <XAxis dataKey="month" tick={{ fontSize: 10, fontWeight: 700, letterSpacing: 1 }} axisLine={false} tickLine={false} />
                <YAxis hide />
                <Tooltip
                  cursor={{ fill: "var(--color-stone)" }}
                  contentStyle={{ background: "var(--color-background)", border: "1px solid var(--color-border)", borderRadius: 0, fontSize: 11 }}
        ...
        {/* Recent ledger */}
        <div className="col-span-12 lg:col-span-5 bg-background border border-border overflow-hidden">
          <div className="p-4 border-b border-border bg-foreground text-background flex justify-between items-center">
            ...
            <Link to="/app/transactions" className="text-[10px] opacity-70 hover:opacity-100">VIEW ALL →</Link>
          </div>
              ...
              {txns.slice(0, 5).map(t => (
                <tr key={t.id}>
                  ...
                  <td className="py-3 px-2">
                    <div className="text-xs font-medium truncate max-w-[160px]">{t.description}</div>
                    <div className="text-[10px] text-muted-foreground uppercase tracking-wider">{CATEGORY_LABEL[t.category]}</div>
                  </td>
      ...
      <div className="mt-12 grid md:grid-cols-2 gap-6">
        <Link to="/app/advisor" className="bg-foreground text-background p-6 flex items-center justify-between hover:bg-accent transition-colors group">
          <div>
            <p className="label-eyebrow text-background/60 mb-2">AI Advisor</p>
            <h3 className="font-display text-xl font-bold tracking-tight">Ask anything about your money →</h3>
          </div>
        </Link>
        <Link to="/app/transactions" className="bg-accent text-background p-6 flex items-center justify-between hover:opacity-90 transition-opacity">
          <div>
            <p className="label-eyebrow text-background/70 mb-2">Ledger</p>
            <h3 className="font-display text-xl font-bold tracking-tight">Add or import transactions →</h3>
          </div>
        </Link>
      </div>
    </div>
  ...
  return (
    <div className="relative size-48">
      <svg width="192" height="192" viewBox="0 0 192 192" className="-rotate-90">
        <circle cx="96" cy="96" r={radius} fill="none" stroke="var(--color-stone)" strokeWidth="12" />
        <circle
          cx="96" cy="96" r={radius}
          fill="none" stroke="var(--color-brand)" strokeWidth="12"
  ...
  return (
    <div className="px-8 py-24 max-w-3xl mx-auto text-center animate-reveal">
      ...
      <h1 className="font-display text-5xl font-black tracking-tighter mb-6">
        Let's architect<br />your numbers.
      </h1>
      <p className="text-muted-foreground mb-10 max-w-md mx-auto">
        Upload a CSV from your bank or add transactions manually to see your financial health score, spending breakdown, and AI insights.
      </p>
      <Link to="/app/transactions" className="inline-block px-6 py-3 bg-foreground text-background text-sm font-bold tracking-wide hover:bg-accent transition-colors">
