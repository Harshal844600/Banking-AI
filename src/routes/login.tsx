    ...
    <div className="min-h-screen grid md:grid-cols-2 bg-background text-foreground">
      <aside className="hidden md:flex flex-col justify-between p-12 bg-foreground text-background">
        <Link to="/" className="flex items-center gap-2">
          <div className="size-8 bg-background text-foreground flex items-center justify-center font-display font-black text-xl italic">F</div>
        ...
        <p className="text-[10px] font-bold tracking-[0.2em] text-background/40">ENCRYPTED END-TO-END</p>
      </aside>

      <main className="flex items-center justify-center p-8">
        <div className="w-full max-w-sm">
          <p className="label-eyebrow text-muted-foreground mb-3">{isLogin ? "Returning user" : "New account"}</p>
          ...
          <p className="mt-6 text-xs text-muted-foreground">
            {isLogin ? (
              <>No account? <Link to="/signup" className="font-semibold text-foreground hover:text-accent">Sign up</Link></>
            ) : (
              <>Already have one? <Link to="/login" className="font-semibold text-foreground hover:text-accent">Log in</Link></>
            )}
          </p>
        </div>
      </main>
...
function GoogleIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" aria-hidden>
      ...
      <path fill="#FBBC05" d="M5.84 14.09a6.97 6.97 0 010-4.18V7.07H2.18a11 11 0 000 9.86l3.66-2.84z"/>
